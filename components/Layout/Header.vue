...
<template>
  <header class="sticky top-0 z-50 bg-white border-b border-primary shadow-sm">
      ...
      <nav class="hidden lg:flex items-center space-x-8">
        <div v-for="(item, index) in menuitems" :key="index" class="relative group">
          <div v-if="item.children" class="relative">
            ...
            <ul
              class="absolute left-0 top-full mt-2 min-w-56 bg-white shadow-md rounded-md overflow-hidden transform scale-y-0 group-hover:scale-y-100 transition-transform origin-top duration-300"
              :class="{ 'scale-y-100': submenuOpen === index }"
            >
              <li v-for="child in item.children" :key="child.path" class="border-b last:border-0">
                <NuxtLink
                  :to="child.path"
          ...
          <NuxtLink v-else :to="item.path" class="text-secondary hover:text-gray-900 text-sm">
            {{ item.title }}
          </NuxtLink>
        </div>
      </nav>

      <button @click="open = !open" class="lg:hidden text-secondary focus:outline-none">
    ...
    >
      <ul class="flex flex-col bg-white border-t border-gray-200">
        <li v-for="(item, index) in menuitems" :key="index">
          <div v-if="item.children" class="flex flex-col">
            <div class="flex items-center justify-between gap-2 px-4 py-3">
              <NuxtLink
                v-if="item.clickable && item.path"
                :to="item.path"
                class="text-sm text-secondary hover:text-gray-900"
              >
                {{ item.title }}
              </NuxtLink>
              ...
              <button
                @click="submenuOpen = submenuOpen === index ? null : index"
                class="flex items-center justify-center text-secondary hover:text-gray-900"
                :aria-expanded="submenuOpen === index"
                :aria-label="`${item.title} Untermenü öffnen`"
                type="button"
              >
            ...
            <ul v-show="submenuOpen === index" class="pl-6 bg-gray-50 overflow-hidden transition-all duration-300">
              <li v-for="child in item.children" :key="child.path" class="border-b last:border-0">
                <NuxtLink :to="child.path" class="block px-4 py-2 text-sm text-secondary hover:bg-gray-100">
                  {{ child.title }}
                </NuxtLink>
              </li>
            </ul>
          ...
          <NuxtLink v-else :to="item.path" class="block px-4 py-3 text-sm text-secondary hover:bg-gray-100">
            {{ item.title }}
          </NuxtLink>
        </li>
      </ul>
